...
selection for regression. An \verb0R0 package for fitting linear
models using a GPU already exists - \verb0gputools0. I wrote three
separate functions, based on \verb0gputools0, to fit all possible
sub-models and tested them with a variety of choices of the number of
observations and the number of covariates.
...
probability assigned to model {\it not} included in this list.
